...
c_output_path = os.getenv("C_OUTPUT_PATH", base_dir + "/set/croped/")  # cropされた画像の出力先
...
def main():
    ...
    for file in tqdm(sorted(glob.glob(input_path)), desc="画像解析"):
        input_image = os.path.join(
            execution_path, os.path.dirname(input_path), os.path.basename(file)
        )
    ...
    for file in tqdm(sorted(glob.glob(input_path)), desc="ファイル書き出し"):
        croped_output_image_path = os.path.join(
            execution_path, c_output_path, os.path.basename(file)
        )
